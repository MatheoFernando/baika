 ...
 const t = useTranslations('analytics');
  ...
  const processChartData = useCallback((data: BaseItem[], timeFilter: string): ChartData[] => {
    if (timeFilter === "month") {
      ...
      // NOVO: Agrupar por dia da semana
      const daysOfWeek = ["Seg", "Ter", "Qua", "Qui", "Sex", "Sáb", "Dom"]
      const weekGroups: Record<string, number> = {}
      daysOfWeek.forEach((d) => { weekGroups[d] = 0 })
      data.forEach((item) => {
        if (item.createdAtDate && !isNaN(item.createdAtDate.getTime())) {
          // 1 = Monday, 7 = Sunday (date-fns/locale ptBR)
          const dayIndex = item.createdAtDate.getDay() // 0=Dom, 1=Seg, ..., 6=Sáb
          // Ajustar para começar em segunda
  ...
  return (
    ...
    <Card className="w-full shadow-sm rounded-sm  border-0 bg-gradient-to-br from-red-50 to-white">
          ...
          <CardFooter className="flex-col items-start gap-2 text-sm ">
            <div className="flex gap-2 font-semibold leading-none text-red-700">
              Total: {filteredOccurrences.length} ocorrências
              <TrendingUp className="h-4 w-4" />
            </div>
            <div className="leading-none text-gray-700/70">
              {occurrencesTimeFilter === "month" ? "Distribuição por dia no mês" : "Distribuição por hora no período selecionado"}
            </div>
          </CardFooter>
        </Card>

        <Card className="w-full shadow-sm rounded-sm  border-0 bg-gradient-to-br from-blue-50 to-white">
          <CardHeader className="flex flex-row items-center justify-between pb-4">
            <div className="space-y-1">
          ...
          <CardFooter className="flex-col items-start gap-2 text-sm ">
            <div className="flex gap-2 font-semibold leading-none text-blue-700">
              Total: {filteredSupervisions.length} supervisões
              <TrendingUp className="h-4 w-4" />
            </div>
            <div className="leading-none text-gray-700/70">
              Distribuição por hora no período selecionado
            </div>
          </CardFooter>
        </Card>
      </div>
