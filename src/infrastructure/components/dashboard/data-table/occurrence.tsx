...
const PriorityBadge = ({ priority }: { priority: Notification["priority"] }) => {
  const priorityConfig = {
    BAIXA: { variant: "outline", className: "bg-green-100 text-green-800", label: "Baixa" },
    MEDIA: { variant: "outline", className: "bg-yellow-100 text-yellow-800", label: "Média" },
    ALTA: { variant: "outline", className: "bg-orange-100 text-orange-800", label: "Alta" },
    CRITICA: { variant: "outline", className: "bg-red-100 text-red-800", label: "Crítica" },
  }
  ...
  cell: ({ row }: { row: Row<Notification> }) => {
    return (
      ...
      },
      {
        ...
        cell: ({ row }: { row: Row<Notification> }) => <PriorityBadge priority={row.getValue("priority")} />,
        filterFn: (row: Row<Notification>, id: string, value: string[]) => {
